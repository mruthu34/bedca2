...
async function refresh(){
  await Promise.allSettled([loadBoss(), loadLeaderboard(), loadPoints()]);
}
...
async function loadLeaderboard(){
  ...
  try {
    const rows = await api.get('/boss/leaderboard');
    if (!rows.length) {
...
async function hitBoss(e){
  ...
  try {
    ...
    const res = await api.post('/boss/hit', { points_spent }, { auth: true });
